(function(){
  ...
  function renderInto(el, items, titleText){
    ...
    // Build a simple continuous marquee for the footer (and any container that has class sponsors--footer)
    const isFooter = el.classList && el.classList.contains('sponsors--footer');
    ...
    const makeItem = (it) => {
      ...
      const inner = href
        ? `<a class="sponsor-item" href="${href}" target="_blank" rel="noopener noreferrer">${img}</a>`
        : `<span class="sponsor-item">${img}</span>`;
      return inner;
    };

    if (isFooter){
      // Duplicate items so CSS can scroll by 50% for a seamless loop
      const track = items.concat(items).map(makeItem).join('');
      el.innerHTML = `${title}<div class="sponsor-marquee"><div class="sponsor-track">${track}</div></div>`;
      return;
    }

    // Default (non-footer): simple row of logos
    const logos = items.map(makeItem).join('');
  ...
  async function init(){
    ...
    if (elFooter){
      renderInto(elFooter, shuffle(sponsors), null);
    }
    if (elPage){
