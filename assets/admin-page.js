...
(function(){
  ...
  async function readAttachmentFiles(fileList){
    ...
    for(const file of files){
      ...
      output.push({ label: file.name, url });
    }
    return output;
  }

  async function handleAttachmentsUpload(event){
    try{
      ...
      currentAttachments = [];
      renderAttachmentPreview();
      setItemMessage(err && err.message ? err.message : 'Could not process the attachments.');
    }
  }
  ...
  function applyHandbookAdminUiFixes(){
    ...
    if(!document.getElementById(styleId)){
      ...
      style.id = styleId;
      style.textContent = ''
        + '.hb-admin-workspace{display:grid;grid-template-columns:minmax(420px,520px) minmax(0,1fr);gap:20px;align-items:start;}'
        + '.hb-admin-col{display:grid;gap:18px;align-items:start;}'
        + '.hb-admin-compact-card{margin:0!important;}'
        + '.hb-admin-category-card,.hb-admin-saved-card,.hb-admin-editor-card{padding:20px!important;border-radius:22px!important;}'
        + '.hb-admin-category-card > *:first-child,.hb-admin-saved-card > *:first-child,.hb-admin-editor-card > *:first-child{margin-top:0!important;}'
        + '.hb-admin-category-card h3,.hb-admin-saved-card h3,.hb-admin-editor-card h3{margin-bottom:10px!important;}'
        + '.hb-admin-category-row,.hb-admin-item-row{display:flex;justify-content:space-between;align-items:center;gap:14px;padding:12px 14px;border-radius:16px;border:1px solid rgba(17,24,39,.08);margin-bottom:10px;background:#fff;}'
        ...
        + '#hbSimpleCatList,#hbSimpleItemList{display:grid;gap:0;max-height:380px;overflow:auto;padding-right:2px;}'
        + '#hbSimpleCatMsg,#hbSimpleItemMsg{min-height:18px;margin:6px 0 0;}'
        + '#btnHbSimpleCatSave{display:none!important;}'
        + '#hbSimpleCurrentCategory{display:inline-flex;align-items:center;min-height:36px;padding:0 14px;border-radius:999px;background:#edf5ef;color:#215b3f;font-weight:800;}'
        ...
        + '#hbSimpleImagePreview img{max-width:200px!important;max-height:112px!important;object-fit:cover;border-radius:10px;display:block;}'
        ...
        + '#hbSimpleCatMsg{display:none!important;}'
        + '#btnHbSimpleCatClear{display:none!important;}'
        + '#hbSimpleCatActive{display:none!important;}'
        + 'label:has(#hbSimpleCatActive){display:none!important;}'
        + '.hb-admin-category-card .tiny.muted{display:none!important;}'
        ...
        + '.hb-admin-item-title-row{display:grid!important;grid-template-columns:minmax(260px,1.7fr) minmax(180px,1fr);gap:12px;align-items:end;}'
    ...
    const catList = $id('hbSimpleCatList');
    if(catList && catList.parentElement) catList.parentElement.style.display = 'none';
    ...
    if(titleRow){
      titleRow.classList.add('hb-admin-item-title-row');
      titleRow.style.display = 'grid';
      titleRow.style.gridTemplateColumns = 'minmax(260px,1.7fr) minmax(180px,1fr)';
      titleRow.style.gap = '12px';
      titleRow.style.alignItems = 'end';
    }
    ...
    if(titleField){
      ...
      if(titleWrap){
        ...
        titleWrap.style.minWidth = '260px';
      }
      titleField.style.display = 'block';
      titleField.style.width = '100%';
      titleField.style.minWidth = '260px';
      titleField.style.maxWidth = 'none';
      titleField.style.flex = '1 1 auto';
    }
    ...
    if(statusField){
      ...
      if(statusWrap){
        ...
        statusWrap.style.minWidth = '180px';
      }
      statusField.style.width = '100%';
      statusField.style.minWidth = '180px';
    }
  ...
  async function boot(){
    ...
    $id('btnHbRemoveAttachments')?.addEventListener('click', function(){
      currentAttachments = [];
      if($id('hbSimpleItemAttachments')) $id('hbSimpleItemAttachments').value = '';
      renderAttachmentPreview();
      setItemMessage('Attachments removed from this draft.');
    });
    $id('hbSimpleItemImage')?.addEventListener('change', handleImageUpload);
    $id('hbSimpleItemAttachments')?.addEventListener('change', handleAttachmentsUpload);
