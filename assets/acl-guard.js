...
(function () {
  ...
  function applyNavAcl(acl, role) {
    ...
    links.forEach(a => {
      ...
      // Add/refresh label badge
      if (!a.querySelector('.anw-nav-badge')) {
        const b = document.createElement('span');
        b.className = 'anw-nav-badge';
        b.textContent = label;
        a.appendChild(b);
      } else {
        a.querySelector('.anw-nav-badge').textContent = label;
      }

      // Hide disallowed
      if (allowed.length && !allowed.includes(role)) {
        a.classList.add('anw-acl-hidden');
        a.setAttribute('aria-hidden', 'true');
        a.tabIndex = -1;
      } else {
        a.classList.remove('anw-acl-hidden');
