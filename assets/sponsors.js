/**
 * Aderrig NW — Community Support auto-rotation
 * ------------------------------------------------
...
(function(){
  ...
  function renderInto(el, items, titleText){
    ...
    const note = `<p class="sponsors-note">Community Support help cover hosting and maintenance costs. Supporters have no access to reports, alerts, or resident data.</p>`;
  ...
  async function init(){
    ...
    if (elFooter){
      renderInto(elFooter, shuffle(sponsors).slice(0, CONFIG.footerCount), 'Community Support');
    }
    if (elPage){
      renderInto(elPage, shuffle(sponsors).slice(0, CONFIG.pageCount), 'Community Support');
    }
    if (elDash){
      renderInto(elDash, pickForSession(sponsors, CONFIG.sessionCount), 'Community Support');
    }
    if (elAll){
      renderInto(elAll, sponsors.slice(0, CONFIG.allLimit), 'Community Support');
    }
  }
