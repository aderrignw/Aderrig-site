...
async function addBackupIndexEntry(store, item){
  ...
  await safeSetJson(store, indexKey, idx, { metadata: { updatedAt: new Date().toISOString() } });
}
...
async function createSafetyBackup(store, note){
  ...
  await safeSetJson(store, `anw_backup_${id}`, snapshot, { metadata: { createdAt, kind: "backup", backupType: "safety-before-restore" } });
